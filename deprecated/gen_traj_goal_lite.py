...
def main(args=None):
    ...
    ref_table = Parallel(n_jobs=jobs)(
        delayed(gen_traj)([0, 0, 0], goal_i) for goal_i in tqdm(goals)
    )
    ref_table = np.array(ref_table)

    ref_states = integrate_path_mult(ref_table)
    ref_states = np.array(ref_states)

    init_states = np.zeros((len(goals), 3))
    transformed_states = Parallel(n_jobs=jobs)(
        delayed(transform_traj)(t_i, s_i) for t_i, s_i in tqdm(zip(ref_states, init_states), total=len(init_states))
    )
    ref_states = np.array(transformed_states)

    np.savez(
        'reference_trajectory_table_lite.npz',
        ref_table=ref_table,
